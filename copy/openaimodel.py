...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, pos_emb=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context)
            elif isinstance(layer, SpatialTransformer_gate):
                x = layer(x, context, pos_emb)
            else:
                x = layer(x)
        return x
...
def get_position_embedder(multires, i=0, in_dim=3, include_input=True):
    if i == -1:
...
class UNetModel_toss(nn.Module):
    ...
    def __init__(
        ...
        # camera pose enc
        # frequency
        if self.pose_enc == "freq":
            self.pos_enc_dim = pos_enc_dim
            self.dir_enc_dim = dir_enc_dim
            self.dir_encoder, dir_dim = get_position_embedder(multires=self.dir_enc_dim, in_dim=2, include_input=True)
            self.z_encoder, z_dim = get_position_embedder(multires=self.pos_enc_dim, in_dim=1, include_input=True)
            # embed to time_embed_dim
            self.pose_net = nn.Sequential(
                # linear(z_dim+dir_dim, context_dim),
                linear(136, context_dim),
                nn.SiLU(),
                linear(context_dim, context_dim),
            )
        
        elif self.pose_enc == "identity":
            self.pose_net = nn.Sequential(
        ...
        elif self.pose_enc == "zero":
            self.pose_net = nn.Sequential(
                linear(4, context_dim),
                nn.SiLU(),
                linear(context_dim, context_dim),
            )

        elif self.pose_enc == "vae":
            self.pos_enc_dim = pos_enc_dim
            self.dir_enc_dim = dir_enc_dim
            self.dir_encoder, dir_dim = get_position_embedder(multires=self.dir_enc_dim, in_dim=2, include_input=True)
            self.z_encoder, z_dim = get_position_embedder(multires=self.pos_enc_dim, in_dim=1, include_input=True)
            # embed to time_embed_dim
            self.pose_net = nn.Sequential(
                # linear(z_dim+dir_dim, model_channels),
                linear(136, model_channels),
                nn.SiLU(),
    ...
    def forward(self, x, timesteps=None, context=None, y=None, delta_pose=None, **kwargs):
        ...
        if (self.pose_enc == "freq") or (self.pose_enc == "vae"):
            ...
            dir_embed = self.dir_encoder(delta_pose[:,:1])
            pos_embed = self.z_encoder(delta_pose[:,:1])

            print("dir_embedding:")
            print(dir_embed.shape)
            print(f"pos_embedding: {pos_embed.shape}")
            # pos_emb = self.pose_net(torch.cat([dir_embed, pos_embed], dim=-1)).unsqueeze(1)
            pos_emb = self.pose_net(torch.cat([dir_embed, pos_embed], dim=-1))

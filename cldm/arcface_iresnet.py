# Copyright (c) InsightFace / deepinsight
# SPDX-License-Identifier: Apache-2.0
#
# Vendored from:
# https://github.com/deepinsight/insightface/blob/master/recognition/arcface_torch/backbones/iresnet.py
# (minor edits: module docstring; keep using_ckpt = False for stable training-time forwards)

"""IResNet backbones used by InsightFace arcface_torch (iresnet18–200)."""
...
__all__ = ["IResNet", "IBasicBlock", "iresnet18", "iresnet34", "iresnet50", "iresnet100", "iresnet200"]

# Training the diffusion stack should not use checkpointing inside the frozen ArcFace backbone.
using_ckpt = False
    ...
    def forward(self, x):
        if torch.cuda.is_available():
            if hasattr(torch, "amp"):
                autocast_cm = torch.amp.autocast("cuda", enabled=self.fp16)
            else:
                autocast_cm = torch.cuda.amp.autocast(enabled=self.fp16)
        else:
